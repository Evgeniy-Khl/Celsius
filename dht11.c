...
unsigned char startDHT(void)
{
   ...
   if(Other) delay_ms(30); // MCU Sends out Start Signal to DHT and pull down voltage for at least 18ms to let DHT11 detect the signal.
...
//        delay_us(30);   // When DHT is sending data to MCU, every bit of data begins with the 50us low-voltage-level
    ...
    if(flag==tem[4])
     {
      if(Other) {pvRH=(int)tem[0]*10; pvT =(int)tem[2]*10;}             // Other = 1 -> DHT11
      else {pvRH =(int)tem[0]*256+tem[1]; pvT =(int)tem[2]*256+tem[3];} // Other = 0 -> DHT21
      pvRH +=biasHum;                  // коррекция датчика влажности
      err = 0;
